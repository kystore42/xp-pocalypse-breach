...
type IconId = 'network' | 'cmd' | 'outlook' | 'recycleBin' | 'taskMgr' | 'updateCenter' | 'settings' | 'cooler' | 'icq' | 'defrag' | 'hardwareShop' | 'firewallTetris';
...
function getDefaultIconPositions(): Record<string, { x: number; y: number }> {
  ...
  ICON_DEFS.forEach((def, idx) => {
    positions[def.id] = { x: 16, y: 24 + idx * 72 };
  });
  return positions;
}
...
export default function App() {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none z-[998] opacity-[0.06]"
        style={{
      ...
      <div className="absolute inset-0 pointer-events-none z-[998] opacity-[0.03]"
        style={{
      ...
      <div className="absolute top-2 right-2 z-[100] pointer-events-none flex flex-col gap-2">
        ...
        <div className={`bg-black/60 backdrop-blur-sm rounded px-3 py-2 text-[10px] font-mono border ${
          ...
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className={`font-bold text-[12px] ${breachLevel > 70 ? 'animate-pulse' : ''}`}>
              {breachLevel.toFixed(0)}%
            </span>
          </div>
          <div className="w-32 h-[6px] bg-gray-700 rounded overflow-hidden">
            <div 
        ...
        <div className={`bg-black/60 backdrop-blur-sm rounded px-3 py-2 text-[10px] font-mono border ${
          ...
          <div className="flex items-center gap-2 mb-1">
            <span>🌡️ CPU</span>
            <span className={`font-bold text-[12px] ${cpuTemp > 85 ? 'animate-pulse' : ''}`}>
              {cpuTemp.toFixed(0)}°C
            </span>
          </div>
          <div className="w-32 h-[6px] bg-gray-700 rounded overflow-hidden">
