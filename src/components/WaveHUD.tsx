...
const WaveHUD: React.FC = () => {
  ...
  return (
    <div className="bg-black/70 backdrop-blur-sm rounded-lg px-4 py-3 text-[13px] font-mono border border-purple-500/50 text-purple-300 min-w-[160px]">
      <div className="flex items-center gap-2 mb-1">
        <span>🌊 WAVE</span>
        <span className="font-bold text-[16px] text-purple-200">
          {currentWave + 1}/{totalWaves}
        </span>
      </div>
      <div className="text-[11px] text-purple-400 mb-1">{wave.name}</div>
      {!wavePaused ? (
        <div className="flex items-center gap-1">
          <span>⏱</span>
          <span className={`font-bold text-[14px] ${waveTimeLeft < 30 ? 'text-yellow-400 animate-pulse' : ''}`}>
        ...
        <button
          ...
          className="w-full mt-1 px-3 py-1.5 bg-green-600 text-white text-[12px] font-bold rounded hover:bg-green-500 active:bg-green-700 animate-pulse"
        >
